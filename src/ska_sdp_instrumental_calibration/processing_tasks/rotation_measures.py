...
def _plot_rm(
    rm_peek: np.float64,
    rm_spec: npt.NDArray[np.complex],
...
def model_rotations(
    ...
    invN = 1 / len(gaintable.frequency)
    invR = 1 / np.sqrt(2)
    ...
    updated_gain_xr = gaintable.gain.copy()
    updated_gain_xr.data = updated_gain

    return gaintable.assign({"gain": updated_gain_xr})
