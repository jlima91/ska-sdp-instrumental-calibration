...
@patch("ska_sdp_instrumental_calibration.processing_tasks.lsm.Path")
@patch("ska_sdp_instrumental_calibration.processing_tasks.lsm.pd.read_csv")
@patch("ska_sdp_instrumental_calibration.processing_tasks.lsm.logger")
def test_should_generate_lsm_from_csv_file(
    logger_mock, read_csv_mock, path_mock
):
    ...
    mock_df = pd.DataFrame(
        {
            ...
            "U (Jy)": [0.0],
            "V (Jy)": [0.0],
            "Ref. freq. (Hz)": [2.000000e08],
            "Spectral index": [-3.70882e-01],
            "Rotation measure (rad/m^2)": [0.0],
            "FWHM major (arcsec)": [2.19263e02],
            "FWHM minor (arcsec)": [1.464811e02],
            "Position angle (deg)": [-4.158033e00],
        }
    )
