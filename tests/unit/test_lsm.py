...
@patch("ska_sdp_instrumental_calibration.processing_tasks.lsm.Path")
@patch("ska_sdp_instrumental_calibration.processing_tasks.lsm.logger")
def test_generate_lsm_from_csv_file(logger_mock, path_mock, open_mock):
    """
    Test that a if a csv file is passed to the function generate_lsm_from_csv,
    it reads the file line by line, and returns a list of Components, where
    each component satisfies following criterions:

    1. The flux of the component is greater than minimum flux density
    2. The component is within the field of view
    """
    path_mock.return_value = path_mock
    path_mock.is_file.return_value = True
    ...
    fileobj = MagicMock(name="csv object")
    fileobj.__iter__.return_value = [CSV_LINE]

    open_mock.return_value = open_mock
    open_mock.__enter__.return_value = fileobj

    lsm = generate_lsm_from_csv("sky_model.csv", phasecentre, fov=10)

    open_mock.assert_called_once_with("sky_model.csv", "r")
    fileobj.close.assert_called_once()
...
def test_should_exclude_csv_comp_when_flux_is_less_than_min_flux(
    ...
    """
    Test that for a Component in a csv file, if its flux is less than
    minimum flux density, then its excluded from the list of components.
    """
    path_mock.return_value = path_mock
    path_mock.is_file.return_value = True
    ...
    fileobj = MagicMock(name="sky_model.csv")
    fileobj.__iter__.return_value = [CSV_LINE]
    ...
    lsm = generate_lsm_from_csv(
        "sky_model.csv", phasecentre, fov=10, flux_limit=1.0
    )
...
@patch("builtins.open")
@patch("ska_sdp_instrumental_calibration.processing_tasks.lsm.Path")
def test_exclude_csv_comp_when_its_out_of_fov(path_mock, open_mock):
    """
    Test that for a Component in a csv file, if its position is far away
    from the phase centre (outside of the field of view), then its excluded
    from the final list of components.
    """
    path_mock.return_value = path_mock
    path_mock.is_file.return_value = True
    ...
    lsm = generate_lsm_from_csv("sky_model.csv", phasecentre)
