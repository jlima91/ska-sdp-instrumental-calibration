...
def test_should_smooth_the_gain_solution(sliding_window_smooth_mock):
    ...
    smooth_gain_solution_stage.stage_definition(
        upstream_output, 3, "median", False, plot_config, "./output/path"
    )
...
@patch(
    "ska_sdp_instrumental_calibration.workflow.stages."
    "smooth_gain_solution.apply_weights_on_gain"
)
@patch(
    "ska_sdp_instrumental_calibration.workflow.stages."
    "smooth_gain_solution.sliding_window_smooth"
)
def test_should_smooth_the_gain_solution_with_weights_applied(
    sliding_window_smooth_mock, apply_weights_on_gain_mock
):
    upstream_output = UpstreamOutput()
    ...
    apply_weights_on_gain_mock.return_value = gaintable_mock
    sliding_window_smooth_mock.return_value = gaintable_mock
    ...
    smooth_gain_solution_stage.stage_definition(
        upstream_output, 3, "median", True, plot_config, "./output/path"
    )

    sliding_window_smooth_mock.assert_called_once_with(
        upstream_output.gaintable, 3, "median"
    )
    apply_weights_on_gain_mock.assert_called_once_with(gaintable_mock)
...
def test_should_smooth_the_gain_solution_using_sliding_window_mean(
    ...
    smooth_gain_solution_stage.stage_definition(
        upstream_output, 3, "mean", False, plot_config, "./output/path"
    )
